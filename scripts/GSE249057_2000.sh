#!/bin/bash
# Script to run GSE249057 dataset training
# Example: bash GSE249057_2000.sh
...
# ==========================================
# Configuration: Data processing parameters
# ==========================================
DATA_FILE_NAME=GSE249057_2000

# Update the data path and copy this file to new data path
NEW_DATA_PATH=${DATA_PATH}/${DATA_FILE_NAME}
DEFAULT_ROOT_DIR_BASE=${DATA_PATH}/outputs_GSE249057_2000
...
# ==========================================
# Step 1: Process GSE249057 data (SKIPPED)
# ==========================================
# Data processing step has been removed
# Please ensure the processed data already exists at: $NEW_DATA_PATH
echo "Skipping data processing step..."
echo "  Expected data path: $NEW_DATA_PATH"
echo "=========================================="
echo ""

echo ""
echo "=========================================="
echo "Step 1: Running training scripts..."
echo "=========================================="

# ==========================================
# Step 1.1: Run ADDA training
# ==========================================
if [ "$RUN_ADDA" -eq 1 ]; then
    echo ""
    echo "1.1. Running ADDA training..."
    ...
    echo ""
    echo "1.1. Skipping ADDA training (RUN_ADDA=0)"
fi

# ==========================================
# Step 1.2: Run DANN training
# ==========================================
if [ "$RUN_DANN" -eq 1 ]; then
    echo ""
    echo "1.2. Running DANN training..."
    ...
    echo ""
    echo "1.2. Skipping DANN training (RUN_DANN=0)"
fi

# ==========================================
# Step 1.3: Run DSN training
# ==========================================
if [ "$RUN_DSN" -eq 1 ]; then
    echo ""
    echo "1.3. Running DSN training..."
    ...
    echo ""
    echo "1.3. Skipping DSN training (RUN_DSN=0)"
fi

# ==========================================
# Step 1.4: Run Usual training
# ==========================================
if [ "$RUN_USUAL" -eq 1 ]; then
    echo ""
    echo "1.4. Running Usual training..."
    ...
    echo ""
    echo "1.4. Skipping Usual training (RUN_USUAL=0)"
fi

# CP this file to output path
OUTPUT_PATH=${DEFAULT_ROOT_DIR_BASE}/${DATA_FILE_NAME}.sh
cp $0 $OUTPUT_PATH
