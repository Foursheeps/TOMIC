...
"""
Lightning Module for Adversarial Discriminative Domain Adaptation with Dual Transformer.
...
class LightningModule(BaseLightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.source_encoder = DualTransformerEncoder(
            seq_len=seq_len,
    ...
    def _extract_classification_features(self, features: dict) -> torch.Tensor:
        """Extract features for classification for scGPT model."""
        # features is a dict with 'aligned', 'name', 'expr' keys
        # Use CLS token (first token) from concatenated output for classification
        aligned_output = features["aligned"]  # [batch_size, 1 + seq_len + seq_len, hidden_size]
        return aligned_output[:, 1, :]  # [batch_size, hidden_size]

    def _extract_discriminator_features(self, features: dict) -> torch.Tensor:
        """
        Extract features for discriminator for scGPT model.

        Args:
            features: Dict with 'aligned', 'name', 'expr' keys.

        Returns:
            Features for discriminator of shape [batch_size, hidden_size].
        """
        # Use CLS token (first token) from aligned output for discriminator
        aligned_output = features["aligned"]  # [batch_size, 1 + seq_len + seq_len, hidden_size]
        return aligned_output[:, 0, :]  # [batch_size, hidden_size]
